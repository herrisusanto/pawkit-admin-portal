...
export default function BreedUpdateForm(props) {
  const {
    name: nameProp,
    breed: breedModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = breedRecord
      ? { ...initialValues, ...breedRecord }
      : initialValues;
    setName(cleanValues.name);
    setPetType(cleanValues.petType);
    setCoats(cleanValues.coats ?? []);
    setCurrentCoatsValue("");
    setUndercoatRemoval(cleanValues.undercoatRemoval);
    setDurationUnit(cleanValues.durationUnit);
    setBasicGroomingDuration(cleanValues.basicGroomingDuration);
    setFullGroomingDuration(cleanValues.fullGroomingDuration);
    setErrors({});
  };
  const [breedRecord, setBreedRecord] = React.useState(breedModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = nameProp
        ? (
            await client.graphql({
              query: getBreed.replaceAll("__typename", ""),
              variables: { name: nameProp },
            })
          )?.data?.getBreed
        : breedModelProp;
      setBreedRecord(record);
    };
    queryData();
  }, [nameProp, breedModelProp]);
  React.useEffect(resetStateValues, [breedRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          petType,
          coats,
          undercoatRemoval: undercoatRemoval ?? null,
          durationUnit: durationUnit ?? null,
          basicGroomingDuration: basicGroomingDuration ?? null,
          fullGroomingDuration: fullGroomingDuration ?? null,
        };
        ...
        try {
          ...
          await client.graphql({
            query: updateBreed.replaceAll("__typename", ""),
            variables: {
              input: {
                name: breedRecord.name,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "BreedUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(nameProp || breedModelProp)}
